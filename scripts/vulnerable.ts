...
import { getSigners, getTime, getDomain } from './utils';
dotenv.config()

let owner: Wallet, other0: Wallet, other1: Wallet;
const { VULNERABLE_ADDRESS } = process.env

async function makeSig(owner: Wallet, spender: Wallet, value: string, deadline: number) {
  const domain = getDomain('Vulnerable Token', VULNERABLE_ADDRESS as string)
